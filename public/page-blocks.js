    ...
    [document.querySelector('main'), document.querySelector('footer')].forEach(function (root) {
      ...
      if (hasText) {
        eachText(root, function (n) {
          // when the visitor reads Japanese the node may already be translated; match on its Thai original
          var I = window.MariI18n, orig = I && I.originalOf ? I.originalOf(n) : null;
          var v = orig != null ? orig : n.nodeValue;
          var k = norm(v);
          if (Object.prototype.hasOwnProperty.call(texts, k)) {
            var lead = v.match(/^\s*/)[0], trail = v.match(/\s*$/)[0];
            var next = lead + texts[k] + trail;
            if (I && I.setThai) I.setThai(n, next); else n.nodeValue = next;
          }
        });
      }
      if (hasImg) {
