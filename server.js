...
const server=http.createServer(async(req,res)=>{
 try{
  ...
  let file=req.url==='/'?'/index(2).html':decodeURIComponent(req.url.split('?')[0]); if(file.includes('..'))return json(res,400,{error:'bad path'});let fp=path.join(ROOT,file);if(!fs.existsSync(fp)||fs.statSync(fp).isDirectory())fp=path.join(ROOT,'index(2).html');const ext=path.extname(fp),types={'.html':'text/html; charset=utf-8','.js':'text/javascript; charset=utf-8','.css':'text/css; charset=utf-8','.json':'application/json'};res.writeHead(200,{'Content-Type':types[ext]||'application/octet-stream'});fs.createReadStream(fp).pipe(res)
