# V18 Chat Game Update

> `RPG` remains the page/menu name and `/rpg.html` remains the URL. The old RPG engine has been replaced with a MongoDB-backed Chat Game.

- No canvas movement
- No EXP/levels/HP/MP/stats
- No inventory/equipment
- No monsters or battle system
- Chat rooms, online presence, chat history, exploration/travel/talk/search/rest actions
- Thai/Japanese toggle
- Mobile responsive
- Existing login, wallet, purchase access (`rpgOwned`), and main website systems are preserved.
...
6. Replace `<username>` and `<password>` with the database user from step 3, and add a database name right after `.net/`, e.g.:
   ...
   ```
7. On Cloudflare: your Worker → **Settings → Variables and Secrets** → add `MONGODB_URI` (as a **Secret**) with that full string as the value → **Save**.
...
To turn this on, get your RCON host/port/password from your hosting panel (Multicraft, Pterodactyl, etc. all expose this) and set `RCON_HOST`, `RCON_PORT`, and `RCON_PASSWORD` as Secrets on your Worker (Cloudflare dashboard → Worker → Settings → Variables and Secrets). **Note:** the RCON port is almost always different from the port players connect on — don't reuse `MC_PORT` here.
...
## Page-load animation
Every public page (`index`, `promo`, `vip`, `rules`, `team`, `topup`, `minigames`, `chat`, `auth`) now shows a brief full-screen loading animation — a running character — while the page's assets load, then fades out automatically (`window.load`, minimum ~450ms so it never just flashes). Purely CSS/SVG, no images or libraries, defined inline right after `<body>` in each page.

## Fix: menu clicks going to the wrong place on every page except index.html
Every page loaded a small script meant to make `go('topup')`, `go('vip')`, etc. jump to the right file when clicked from a page where that section doesn't exist (e.g. clicking "VIP" while on `rules.html`). That script set `window.go = ...` *before* the page's own `function go(id){...}` was defined further down the same page — and since a `function` declaration always wins over an earlier plain assignment to the same name, the fallback got silently wiped out the instant the page finished parsing, on every page, index.html included. In practice this only showed up as broken navigation on the non-home pages, because index.html happens to contain the actual `#server`/`#discord` sections `go()` can scroll to, while pages like `promo.html` or `vip.html` don't contain any of those ids, so clicks either did nothing or (if an unrelated element happened to share that id) jumped to the wrong spot.
Fix: the redirect-to-another-page fallback is now built directly into the one `go(id)` function itself (no separate script trying to patch it later), so every menu button behaves the same way on every page: scroll to the section if it exists on the current page, otherwise navigate to the right file.

## 🎨 สีชื่อในเกม (`/namecolor.html`) - sold as packages
Follows the Mari SMP price list. Players buy a package once with wallet credit, then color their **own** Minecraft name from the site; the server runs `nick <player> <colored name>` (EssentialsX) over the same RCON / Pterodactyl console the shop uses.

| Package | Default price | What it unlocks |
|---|---|---|
| `legacy` 7 สีเลกาซี | ฿20 | one of 7 colors: `&c &d &a &6 &4 &b &e` |
| `thai` ชื่อสีภาษาไทย | ฿30 | all 16 named colors (Thai names), bold/italic/underline/strike, colored tag |
| `rgb` ชื่อสี RGB กำหนดเอง | ฿50 | any `#RRGGBB`, 2-color gradient, custom tag text + RGB tag color |

- Prices, labels and on/off per package are editable in `/admin.html` → 🎨 สีชื่อ (the price list says 50-60 for RGB: set 60 if you want the top of the range). Upgrading pays only the difference. Each purchase is logged as an order (`NC-…`).
- **Who gets what**: an admin override on the account wins; otherwise the best of *global default*, *website-title level* (free grant, e.g. supporters) and the *purchased package*. Overrides can also block someone (`none`).
- **Display name**: by default the player's own bound Minecraft name is colored (names with spaces are rejected). The `rgb` package may also type a different display name. Rules: `[A-Za-z0-9_]` (Thai optional - also needs EssentialsX `allowed-nick-regex`), 3-16 chars (global max + per-user override; EssentialsX `max-nick-length` defaults to 15, raise it there too), not on the blocklist (admin, mod, owner, ... also catches `Adm1n` / `a_d_m_i_n`), and unique against every other account's Minecraft name, site username, display name and pending request. A new/changed name waits for admin approval by default (switchable); admins approve (runs the commands) or reject with a reason the player sees, and a rejection gives the cooldown back. Changing only the color never needs approval.
- **Staff-reserved words**: `admin`, `mod`, `owner` (ผู้สร้างเซิร์ฟเวอร์) and Thai equivalents (แอดมิน, ผู้สร้าง, ม็อด, เจ้าของ, ทีมงาน ...) can't be used in a tag or display name by anyone except accounts an admin has ticked "🛡️ may use reserved words" (per-user, in 🎨 สีชื่อ → สิทธิ์รายบุคคล). Lists are editable (`tagBlocked`, `customNameBlocked`). Matching folds case, `_ .`, look-alikes (Adm1n, @dmin) and Thai tone marks (แอดมิ้น); words of 3 letters or fewer only match as a whole word / `_`-part (so `Mod`, `Mari_Mod` blocked, `Model` fine). To give staff a tag: set their level to `rgb` (or let them buy it), tick the box, and raise their tag length if needed.
- **Tag** (`[ผู้เล่น]`, `[V_V]`, `[MOD]`): OFF by default. When enabled it is set as a LuckPerms prefix (`lp user {player} meta setprefix 100 "{prefix}"`, editable) and only shows if your chat format prints the prefix. Thai package = fixed default text; RGB package = custom text (letters/digits/_/Thai, max length admin-set globally and per user - "admin can extend the length").
- Other admin rules: cooldown, minimum brightness for RGB, formats on/off, require verified MC id, hide individual named colors (black is off by default), command templates, RGB code style (`&#RRGGBB` or `&x&R&R&G&G&B&B`).
- **No console configured**: requests queue under "รอตั้งค่าในเกม" with the exact commands; press "ตั้งค่าแล้ว" afterwards.
- Everything is re-validated server-side; a failed console command rolls back the cooldown and saves nothing. Purchases are one atomic update (balance check + deduction + ownership).

API: `GET /api/namecolor/status`, `POST /api/namecolor/{buy,apply,reset}`; admin (`x-admin-key`): `GET /api/admin/namecolor`, `PUT /api/admin/namecolor/settings`, `GET /api/admin/namecolor/users`, `POST /api/admin/namecolor/users/:id/{access,reserved,tag-max,name-max,approve,reject,reset,done}`. Settings live in `settings` (`id: 'nameColor'`); per-user fields: `nameColorPackage`, `nameColor`, `nameColorAccess`, `nameColorReservedOk`, `nameColorTagMax`, `nameColorNameMax`, `nameColorReject`, `nameColorPending`, `nameColorChangedAt`.


## Mari RPG (separate module)
- Page: `/rpg.html`
- As described at the top of this file, this is the MongoDB-backed Chat
  Game (chat rooms, presence, exploration/travel/talk/search/rest) - not
  a canvas game with stats/inventory/monsters/combat.
- Existing login, wallet, and `rpgOwned` purchase access are preserved.
- Player character, position, online presence and RPG chat are stored in MongoDB.

## TikTok follower count (OAuth v2)

This build adds live TikTok follower statistics to `team.html` for the `mari` and `tann_top` slots.

### Environment variables

Set these under your Worker's **Settings → Variables and Secrets** on Cloudflare (see `CLOUDFLARE_SETUP.md`):

- `TIKTOK_CLIENT_KEY` — TikTok Developer Client Key
- `TIKTOK_CLIENT_SECRET` — TikTok Developer Client Secret (server-side only)
- `TIKTOK_REDIRECT_URI` — exact HTTPS callback registered in TikTok Developer, e.g. `https://YOUR-DOMAIN/api/admin/tiktok/callback`

In TikTok for Developers, enable Login Kit/Web and request the `user.info.stats` scope. The account owner must authorize the app. TikTok's v2 User Info API exposes `follower_count` under `user.info.stats`.

After deploying, sign in with an Admin/Creator account, open `admin.html` → `🎵 TikTok Stats`, and connect each slot. The access/refresh tokens are stored only in MongoDB on the server; they are never sent to the browser. `team.html` reads only the public statistics endpoint.
