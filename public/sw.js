const CACHE='mari-pwa-v32-cloudflare';
const SHELL=['/i18n-jp.js?v=6','/mari-global-ui.js?v=11','/page-blocks.js','/manifest.webmanifest','/account-session.js?v=8'];
...
async function freshHtml(request){
  ...
  html=html.replace(/\/account-session\.js\?v=(?:2|3|4|5|6|7|8)/g,'/account-session.js?v=8');
  if(!html.includes('/account-session.js')){
    const tag='<script src="/account-session.js?v=8" defer></script>';
    if(/<\/body>/i.test(html))html=html.replace(/<\/body>/i,tag+'</body>');else html+=tag;
  }
  const headers=new Headers(response.headers);
  headers.set('Cache-Control','no-store,no-cache,must-revalidate');
  headers.set('X-Mari-Session-Loader','v32-cloudflare');
...
self.addEventListener('fetch',e=>{
  ...
  e.respondWith(caches.match(e.request).then(c=>c||fetch(e.request).then(r=>{const copy=r.clone();caches.open(CACHE).then(x=>x.put(e.request,copy));return r;}).catch(()=>caches.match(e.request))));
});
